...
def lambda_handler(event, context):
    ...
    try:
        ...
        if cost_summary["total_cost"] > cost_threshold:
            ...
            send_slack_alert(
                slack_webhook_url, cost_summary, cost_threshold, environment
            )
            alert_sent = True
        else:
            alert_sent = False
    ...
    try:
        ...
        for group in results.get("Groups", []):
            ...
            if service_cost > 0:  # Only include services with actual costs
                services.append({"name": service_name, "cost": service_cost})
...
def send_slack_alert(
    webhook_url: str, cost_summary: Dict, threshold: float, environment: str
):
    """
